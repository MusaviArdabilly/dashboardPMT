<template>
  <v-dialog v-model="coverage_bts" width="500" persistent>
    <v-card class="pa-5">
      <v-row>
        <v-col lg="11">
        ...
        <v-col lg="1">
          <v-icon @click="closeCoverageBts" class="close-ic">mdi-close</v-icon>
        </v-col>
      </v-row>
...
export default {
  ...
  methods: {
    closeCoverageBts() {
      this.$emit("closeCoverageBts", false);
    },
    initialize() {
      this.getProvince();
      this.getCity();
