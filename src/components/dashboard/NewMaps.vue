...
export default {
    ...
    this.map.on("singleclick", (evt) => {
      ...
      this.map.forEachLayerAtPixel(evt.pixel, (layer) => {
        ...
        if (layer.get("title") != "OSM") {
          console.log(layer.getSource().getParams().title);
          const url = layer
            .getSource()
              ...
              .then((data) => {
                ...
                switch (layer.getSource().getParams().title) {
                  case "coverage":
                    for (let item of data.features) {
                      ...
                      let show =
                        ...
                        ", " +
                        item.properties.namobj +
                        ", " +
                        item.properties.kls_sign +
                        ", " +
                        item.properties.wadmkc +
                        ", " +
                        item.properties.wadmkk +
                        ", " +
                        item.properties.wadmpr;
                  ...
                  case undefined:
                    for (let item of data.features) {
                      // console.log(data.features.getGeometry().getCoordinates());
                      if (item.properties.wadmkk != undefined) {
                        let show =
                          item.properties.kdepum +
                          ", " +
                          item.properties.luas_km +
                          ", " +
                          item.properties.wadmkk +
                          ", " +
                          item.properties.wadmpr;
  ...
  methods: {
    ...
    coverageLayer(value) {
      if (value.category__gn_description.includes("Pemukiman")) {
